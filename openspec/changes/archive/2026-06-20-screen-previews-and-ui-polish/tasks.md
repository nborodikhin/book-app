...
- [x] 3.3 Extract a private `BookmarksScreenContent(books, onBookClick)` overload in `BookmarksScreen.kt`; wire the Hilt entry-point composable through it
...
- [x] 5.2 Replace `SearchScreenTest` with tests using `createComposeRule` + `SearchScreenContent`: loading spinner visible; error message visible; results list visible; bookmark icon filled vs outline
- [x] 5.3 Replace `BookmarksScreenTest` with tests using `createComposeRule` + `BookmarksScreenContent`: empty state message shown; list items rendered when non-empty
